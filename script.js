...
function bookCardComponent(serialNumber, title, sub, text, readMoreButton) {
  return `
...
function booksComponent(logo, cards, button) {
  let cardSectionHtml = '';
  for (let i = 0; i < cards.length; i++) {
    cardSectionHtml += bookCardComponent(i + 1, cards[i].title, cards[i].sub, cards[i].text, button);
  };
  return `
  <navbar id="header">
    <h1>${logo}</h1>
    <span class="material-symbols-outlined burger-menu">menu</span>
  </navbar>
  <section>
...
rootElement.insertAdjacentHTML('afterbegin', booksComponent(books.logo, books.cards, books.button));